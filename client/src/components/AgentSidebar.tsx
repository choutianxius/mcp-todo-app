...
export function AgentSidebar({ isOpen, onTodosChange }: AgentSidebarProps) {
  const [messages, setMessages] = useState<AgentMessage[]>([
    {
      id: '0',
      role: 'agent',
      content: 'Hi! I\'m your MCP-powered todo agent. I can help you manage your todos through natural language. Try asking me to list, create, complete, or delete todos!',
      timestamp: Date.now(),
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const agentResponse = await agentService.processPrompt(userMessage.content);
                  ...
                  >
                    ...
                    {message.toolCalls && message.toolCalls.length > 0 && (
                      <div className="mt-2 border-t-2 border-sidebar-border pt-2">
                        <p className="font-mono text-xs text-sidebar-foreground/60">
                          Tools used: {message.toolCalls.map(tc => tc.toolName).join(', ')}
                        </p>
                      </div>
                    )}
