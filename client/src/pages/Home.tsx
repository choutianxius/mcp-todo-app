/**
 * Home Page - Main Todo App
 * Design: Neo-brutalism - asymmetric layout with bold headers
...
export default function Home() {
  ...
  const loadTodos = async () => {
    try {
      ...
      if (tool) {
        const result = await tool.execute({ filter: 'all' });
        setTodos(result);
      }
    } catch (error) {
  ...
  const handleAddTodo = async (title: string, description?: string, tags?: string[]) => {
    try {
      ...
      if (tool) {
        await tool.execute({ title, description, tags });
        await loadTodos();
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="border-4 border-primary bg-card p-8 brutal-shadow-cyan">
          <p className="font-mono text-sm text-foreground">Initializing database...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="relative flex flex-1 overflow-hidden">
        {/* Main Content */}
        <main className={`container mx-auto flex-1 px-4 py-8 transition-[margin] duration-150 ease-out ${isAgentOpen ? 'sm:mr-[400px] lg:mr-[500px]' : ''}`}>
