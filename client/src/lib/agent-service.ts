...
export class AgentService {
  ...
  async processPrompt(prompt: string): Promise<AgentMessage> {
    ...
    try {
      // List todos
      if (this.matchesPattern(lowerPrompt, ['list', 'show', 'get', 'what', 'display'])) {
        ...
        if (tool) {
          ...
          if (result.length === 0) {
            ...
            responseText += result.map((t: any, i: number) => 
          ...
          if (todos.length === 0) {
            ...
            if (todoMatch) {
              ...
              if (toggleTool) {
                ...
                toolCalls.push({ toolName: 'toggle_todo', args: { id: todoMatch.id }, result });
                responseText = `Marked "${todoMatch.title}" as complete!`;
              }
            } else {
              responseText = 'Which todo would you like to complete? Try: "Complete the first one" or mention the todo title.';
            }
          }
        }
      }
      // Delete todo
      else if (this.matchesPattern(lowerPrompt, ['delete', 'remove', 'clear'])) {
        if (this.matchesPattern(lowerPrompt, ['completed', 'done', 'finished'])) {
          ...
          if (listTool) {
            ...
            if (todoMatch) {
              ...
              if (deleteTool) {
                ...
                toolCalls.push({ toolName: 'delete_todo', args: { id: todoMatch.id }, result });
                responseText = `Deleted "${todoMatch.title}".`;
              }
            } else {
              responseText = 'Which todo would you like to delete? Try: "Delete the first one" or mention the todo title.';
            }
          }
        }
      }
      // Help / capabilities
      else if (this.matchesPattern(lowerPrompt, ['help', 'what can', 'how', 'capabilities'])) {
...
Or type "help" to see all my capabilities!`;
      ...
      if (toolCalls.length > 0) {
        toolCalls[toolCalls.length - 1].error = error instanceof Error ? error.message : 'Unknown error';
      }
    }
  ...
  private extractFilter(prompt: string): 'all' | 'completed' | 'pending' {
    ...
    if (prompt.includes('pending') || prompt.includes('active') || prompt.includes('incomplete')) {
      return 'pending';
    }
    return 'all';
  }
